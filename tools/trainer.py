...
class Trainer:
    ...
    def __init__(
        ...
        self.device = device
        self.num_classes = \
            ex.current_run.config['model']['num_classes']
    ...
    def do_epoch(
        ...
        for i, (input, target, _) in enumerate(tqdm_train_loader):
            ...
            if alpha > 0:  # Mixup augmentation
                ...
                mixed_input = lam * input \
                              + (1 - lam) * input[rand_index]

                output = model(mixed_input)
                loss = self.cross_entropy(output,
                                          target_a) * lam \
                + self.cross_entropy(output, target_b) * (1. - lam)
            else:
                output = model(input)
            ...
            if not disable_tqdm:
                tqdm_train_loader.set_description('Acc '
                                                  '{:.2f}'.format(top1.avg))
            ...
            if i % print_freq == 0:
                ...
                if callback is not None:
                    callback.scalar('train_loss',
                        i / steps_per_epoch + epoch, losses.avg,
                        title='Train loss')
                    callback.scalar('@1',
    ...
    def smooth_one_hot(
        ...
        with torch.no_grad():
            new_targets = torch.empty(size=(targets.size(0),
                                                                    self.num_classes),
                                                                    device=self.device)
            new_targets.fill_(label_smoothing /
                                            (self.num_classes-1))
            new_targets.scatter_(1, targets.unsqueeze(1),
                                                        1. - label_smoothing)
        return new_targets
    ...
    def meta_val(
        ...
        with torch.no_grad():
            tqdm_test_loader = warp_tqdm(self.val_loader,
                                         disable_tqdm)
            for i, (inputs, target, _) in enumerate(tqdm_test_loader):
                inputs, target = inputs.to(self.device), \
                                 target.to(self.device, non_blocking=True)
                ...
                train_out = train_out.reshape(meta_val_way,
                                              meta_val_shot, -1).mean(1)
                train_label = train_label[::meta_val_shot]
                prediction = self.metric_prediction(train_out,
                                                    test_out, train_label)
                acc = (prediction == test_label).float().mean()
                top1.update(acc.item())
                if not disable_tqdm:
                    tqdm_test_loader.set_description('Acc '
                                                     '{:.2f}'.format(top1.avg * 100))

        if callback is not None:
            callback.scalar('val_acc',
                                        epoch + 1,
                                        top1.avg,
                                        title='Val acc')
        return top1.avg
