...
class DatasetFolder(object):
    ...
    def __init__(
        ...
        with open(split_file, 'r') as f:
            split = [x.strip().split(',')
                     for x in f.readlines()[1:] if x.strip() != '']
    ...
    def __getitem__(self, index):
        assert os.path.isfile(os.path.join(self.root, self.data[index])), \
            os.path.join(self.root, self.data[index])
        img = Image.open(os.path.join(self.root,
                                      self.data[index])).convert('RGB')
